...
public class PulseGenerator implements Runnable
{
  ...
  private void generatePCM(int pulseWidth, int negPulseWidth, int pulseInterval, int volume, int modulation, short buffer[], int bufferLength, int lastChanged)
  {
    ...
    while (i < bufferLength)
    {
      ...
      if (lastChanged % 2 == 0)
       {
           // What I don't get with this code is there is about 1/50 positive time for the positive pulse then 49/50 negative time just as loud
           // if the volume was decreased it would make sense but as current how is one meant to filter the negative signal from the positive generator
           // from the negative signal from the negative pulse generator and vice versa...? If -volume was decreased significantly then the sound card wouldn't
           // go dc but positive and negative signals could still be combined and filtered at the other end (and frankly the card is already dc by integration -
           // the dc part is what I'm using to control the transistor (dumped ac by capacitor to ground)
  ...
  {
    return (( pulseWidthArray[i] - MIN_PULSE_WIDTH ) * 100) / ( MAX_PULSE_WIDTH - MIN_PULSE_WIDTH );
  }
